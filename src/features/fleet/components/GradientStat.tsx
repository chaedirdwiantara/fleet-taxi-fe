...
export function GradientStat({ label, value, icon: Icon, gradient, sub }: GradientStatDef) {
  return (
...
export function GradientStatRow({ cards }: { cards: GradientStatDef[] }) {
  return (
    <div className={`grid gap-3 sm:grid-cols-2 ${cards.length >= 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'}`}>
      {cards.map((c) => (
