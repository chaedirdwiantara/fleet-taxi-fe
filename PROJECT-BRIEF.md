...
- **Monthly calculated target** ("Total Due") = **Σ of the `due` rows actually imported for that month**, with bebas-setoran days waived (an `is_bebas_setoran = 1` exception or a Rental Monitoring booking covering that date is neither billed nor credited). It is *never* extrapolated: days that have not elapsed, were not imported, or precede the plate's first billed day contribute nothing, and a plate with no `due` rows at all owes **Rp 0** — `fleet_targets.fleet_target` then only sets the *displayed* daily rate, not an obligation. This is the same aggregate that backs Outstanding, so `outstanding_month ≡ calculated_target − paid` holds by construction. Rationale: the daily rate can change mid-month, so multiplying one rate across a window mis-bills; and a driver who joins mid-month owes nothing for the days before they joined.
