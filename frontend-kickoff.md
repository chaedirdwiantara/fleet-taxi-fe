...
3. **Assume the backend may not be ready yet — generate a typed client + mock everything.**
   ...
   - Until the real API responds, run against **[MSW](https://mswjs.io/) mocks** that return the **§6 shapes wrapped in the standard envelope** (§6 "Standard JSON envelope"). MSW is the _single_ mock layer shared by dev-run and tests (§9).
...
**Routing — TanStack Router (recommended) _or_ React Router**
...
export function cellTone(
  ...
  /* pure comparison vs dailyTarget */
}
...
export function unwrap<T>(
  res: { success: true; data: T; meta?: Meta } | { success: false; error: ApiError },
): T {
...
export const qk = {
  ...
  fleet: {
    ...
    importStatus: (platform: 'gojek' | 'grab', id: string) =>
      ['fleet', platform, 'imports', id] as const,
...
export function useImportProgress(platform: 'gojek' | 'grab', importId: string) {
  ...
  useEffect(() => {
    ...
    socket.on('import:failed', (p) =>
      qc.setQueryData(qk.fleet.importStatus(platform, importId), (old: any) => ({
        ...old,
        status: 'failed',
        error: p.error,
      })),
...
_End of frontend-kickoff.md — companion to PROJECT-BRIEF.md (the source of truth). If they disagree, the brief wins._
