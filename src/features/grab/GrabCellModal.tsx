...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`flex justify-between px-4 py-2.5 text-sm ${highlight ? 'bg-emerald-50 dark:bg-emerald-950/40' : 'odd:bg-muted/40'}`}>
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md p-0">
        ...
        {detail.isPending && <p className="px-4 py-6 text-sm text-muted-foreground">Memuat detail…</p>}
        {detail.isError && (
        ...
        {detail.isSuccess && (
          <div className="pb-2">
            ...
            <Row label="Cancel by Driver" value={detail.data.cancelByDriver.toLocaleString('id-ID')} />
            <Row label="Fulfillment Rate" value={`${(detail.data.fulfillmentRate * 100).toFixed(1)}%`} />
            <Row label="Cancellation Rate" value={`${(detail.data.cancellationRate * 100).toFixed(1)}%`} />
            ...
            <Row label="Total Earning Collected" value={formatRupiah(detail.data.earning)} highlight />
          </div>
        )}
      </DialogContent>
    </Dialog>
