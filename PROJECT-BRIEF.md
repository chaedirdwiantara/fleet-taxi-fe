...
- **`due`** rows are the billing record: each one is an amount actually charged for that date. They establish the vehicle's activity range, sum into the monthly target, and feed the **displayed daily rate** (see below).
...
- **Monthly calculated target** ("Total Due") = **Σ of the `due` rows actually imported for that month**, with bebas-setoran days waived (an `is_bebas_setoran = 1` exception or a Rental Monitoring booking covering that date is neither billed nor credited). It is _never_ extrapolated: days that have not elapsed, were not imported, or precede the plate's first billed day contribute nothing, and a plate with no `due` rows at all owes **Rp 0** — `fleet_targets.fleet_target` then only sets the _displayed_ daily rate, not an obligation. This is the same aggregate that backs Outstanding, so `outstanding_month ≡ calculated_target − paid` holds by construction. Rationale: the daily rate can change mid-month, so multiplying one rate across a window mis-bills; and a driver who joins mid-month owes nothing for the days before they joined.
- **Displayed daily rate** per vehicle: `fleet_target` from the targets table if set (>0); else the **mode** of the observed daily dues (ties resolve to the later day, so a new rate wins over the one it replaced); else fallback **488000** (IDR). Display and cell-tone baseline only — it never multiplies into money.
- **Marker rule is separate from the waiver rule**: **spreadsheet money wins** — a day with actual money in the sheet shows no exception marker — but the waiver above still applies to the amount.
- **All-time Outstanding** (the headline number): running balance `Σ|due| − Σ|deduction + manual payment|` over every import for that plate up to the **end of the selected month**, so a past month shows the balance as it stood then. Bebas-setoran and rental days are excluded from **both** sides. Both counted and uncounted manual payments settle the debt — the setoran flag only decides whether the money counts toward the month's omset. Exited plates ("driver keluar") report under their own card instead of the main total.
...
- **DNS/CDN:** **Cloudflare** (same as legacy). Domain **`fleet-taxi.id`** — the apex serves the frontend, **`api.fleet-taxi.id`** the backend.
